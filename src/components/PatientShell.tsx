...
/**
 * Shell layout for the patient portal. It mirrors the original Shell
 * component but presents a tailored navigation and header suitable for
 * patients. Each nav item maps to a dedicated route under /patient.
 */
const NAV_ITEMS = [
  { key: "home", href: "/patient", icon: Activity, label: "Home" },
  { key: "appointments", href: "/patient/appointments", icon: Calendar, label: "Appointments" },
  { key: "history", href: "/patient/appointments/history", icon: History, label: "History" },
  { key: "records", href: "/patient/medical-records", icon: FileText, label: "Medical Records" },
  { key: "lab", href: "/patient/lab-reports", icon: FlaskConical, label: "Lab Reports" },
  { key: "meds", href: "/patient/medicine-center", icon: Pill, label: "Medicine Centre" },
  { key: "tracker", href: "/patient/health-tracker", icon: HeartPulse, label: "Health Tracker" },
  { key: "assistant", href: "/patient/assistant", icon: BrainCircuit, label: "AI Assistant" },
  { key: "emergency", href: "/patient/emergency", icon: Siren, label: "Emergency" },
];
...
export default function PatientShell({ children }: { children: React.ReactNode }) {
  ...
  // Determine active nav item based on the current pathname.
      ...
      <div className="border-t border-inherit p-3">
        <div className="flex items-center gap-2.5 rounded-xl p-2 bg-white/50 dark:bg-slate-800/50">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[10px] text-slate-500 dark:text-slate-400">Patient</p>
          </div>
        ...
        <header className="flex items-center gap-3 border-b bg-white/70 dark:bg-slate-900/70 px-4 py-3 sm:px-6 backdrop-blur-xl">
          ...
          <div className="min-w-0">
            <h1 className="truncate font-display text-base font-bold sm:text-lg text-slate-800 dark:text-slate-200">
              {NAV_ITEMS.find((n) => n.key === activeKey)?.label || "Patient Portal"}
            </h1>
          </div>
