...
import { Sun, Moon, Languages, Building2 } from "lucide-react";

/**
 * Settings page for PHC staff. Allows changing language and theme
 * preferences and displays the staff member’s profile along with the
 * centre they are assigned to. This page is a simplified version
 * tailored for non-admin users.
 */
export default function PHCSettingsPage() {
  const { lang, setLang, dark, setDark, user, showToast, centres } = useApp();
  const centre = centres.find((c: any) => c.id === user.centre);
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <Card className="p-5">
        <SectionTitle icon={Building2}>Profile</SectionTitle>
        <p className="text-sm text-slate-700 dark:text-slate-300">
          <span className="font-semibold text-slate-800 dark:text-slate-200">
            {user.name}
          </span>
          {" "}· {user.role === "manager" ? "Manager" : "Staff"} · {user.email}
        </p>
        {centre && (
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
            Assigned Centre: {centre.name}
          </p>
        )}
        <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
          You are using a demo account. Account management will be
          integrated via Firebase/Supabase.
        </p>
        <button
