...
export const sRand = (n: number) => { const x = Math.sin(n * 9301 + 49297) * 233280; return x - Math.floor(x); };
export const daysLeft = (m: Med) => (m.use > 0 ? Math.floor(m.stock / m.use) : 999);
export const medStatus = (m: Med) => { const d = daysLeft(m); return d <= 3 || m.stock === 0 ? "critical" : d <= 7 || m.stock < m.thr ? "low" : "healthy"; };
export const occPct = (c: Centre) => Math.round((c.occupied / c.beds) * 100);

export function riskScore(c: Centre, meds: Med[]) {
...
export function footfallSeries(c: Centre, hist = 14, fut = 7) {
  const out: any[] = [];
...
export function buildTransfers(meds: Med[]) {
  const recs: any[] = [];
  const byName: Record<string, Med[]> = {};
  meds.forEach((m) => { (byName[m.name] = byName[m.name] || []).push(m); });
  Object.values(byName).forEach((list: Med[]) => {
...
export function buildAlerts(meds: Med[], centres: Centre[], t: any) {
  const cn = (id: string) => centres.find((c) => c.id === id)?.name || id;
  const a: any[] = [];
